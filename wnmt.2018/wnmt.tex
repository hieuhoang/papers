...
We experimented with varying the vocabulary size and the RNN state size before settling for a vocabulary size of 30,000 (for both source and target language) and 256 for the state size, Figure~\ref{tab:BLEU for newstest2014}.
...
10000		& 17.2	& 	& 17.9 \\
20000		& -	&	& 18.4 \\ 
30000		& 17.8	& 18.5	& 18.7 \\ \hline
\end{tabular}
\end{center}
\caption{Test set BLEU (newstest2014) for hyper-parameters}
...
Our second system uses multiplicative-LSTM in the encoder and the first layer of a decder, and a GRU in the second layer. We trained 2 systems with differing vocabulary sizes using an extension of the Nematus toolkit which supports such model.
