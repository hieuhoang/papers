...
The Amun engine implements an efficient batching algorithm that takes into account the actual number of hypotheses that need to be decoded at each decoding step, Figure~\ref{algo:Mini-batching}.
...
%!dtr! original as in the one at the start of this iteration, or original as the one at the start of the process? 
maximum value, $max_b$ is the now higher maximum value, i.e., $max_b > max_a$, and $\Delta = max_a - max_b$. The outline of our function is shown in Algorithm~\ref{algo:Fused Kernel}.
