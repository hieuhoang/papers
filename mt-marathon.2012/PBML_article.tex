...
\begin{equation}
%  score = Sum_i _member_T(weight_i * score_i) + Sum_i _member_L(weight_i * score_i) + weight_wp * word-penalty
\end{equation}
...
\begin{equation}
% score =  Sum_i _member_T(weight_i * score_i)
%    + Sum_i _member_L(weight_i * score_i)
%    + weight_wp * word-penalty
%    + weight_d * score_d
%    + Sum_i _member_R(weight_i * score_i)
%    + future_score
\end{equation}

where $weight_d$ and $score_d$ is the distortion weight and score respectively, R is the set of lexicalized reordering models.
...
\subsection{New Framework}
The new framework better supports the creation of arbitary feature functions by generalizing all code that deals with feature functions.

Equation (1) becomes Equation (...)
\begin{equation}
%  score = Sum_i _member_FF(weight_i * score_i)
\end{equation}
...
The intuition behind this is that most systems will still contain dense features, with a smattering of sparse features. Therefore, it would be more efficient to use dedicated 
