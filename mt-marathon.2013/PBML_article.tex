...
Feature function \emph{state} plays an important role in hypothesis recombination. Two hypotheses can be recombined if their states are equivalent. Higher recombination results in increased effective number of hypotheses created, leading to better model score.

Each feature function can be stateless or stateful, depending on whether it emits a state for each hypothesis. Stateful features imherits from \emph{StatefulFeatureFunction}, otherwise they it inherits from \emph{StatelessFeatureFunction}.
...
cdec and Joshua both have modern feature function frameworks similar to the Moses new
framework. Both of them have no distinction between stateful and stateless features; stateless
features have zero size states.
...
Joshua's decoding algorithm is an implementation of CKY+ \cite{chappelier1998generalized}. It
supports decoding with arbitrary synchronous CFG productions, imposing no limits on the rank of
rules or on the intermingling of terminals and nonterminals.
...
complete rules while building the hypergraph, depicted here with pseudocode:
...
In cdec, each pass creates a hypergraph that is used as the input for the next pass. cdec evaluates stateless feature functions in the first pass, followed by stateful feature functions in the second. Neither decoder partially evaluate stateful feature functions, as done in Moses.
...
\subsection{Moses}
Moses implements supports for both dense and sparse features. Each feature must be identifiable as one or the other.
...
\subsection{cdec}
cdec eschews Moses' dense and sparse hybrid approach and implement only sparse maps. Dense features can still be modelled in the framework as sparse features, whose name are suffixed with indices.
...
The results from the three decoders are comparable, given the same formalism and model data, and adjusting for scaling differences and constants. 
...
Figure~\ref{fig:model} shows the comparison of model scores and BLEU scores for a hierarchical model. Moses and cdec are roughly comparable in terms of speed. Also, Moses and cdec converges to roughly the same model scores, while Joshua makes more search errors, leading to lower BLEU scores.
