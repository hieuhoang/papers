...
We trained a sequence-to-sequence, encoder-decoder NMT system similar to that described in (Rico ???). This uses recurrent neural network with gated recurrent units. The input and output vocabulary size were both set to 85,000 subwords using BPE ??? to adjust the vocabulary to the desired size. The hidden layer dimensions were kept at the default of 512. We used the Marian toolkit to train our models.

For inference, we used and extended Amun (???), the current fastest open-source inference engine for the RNN/GRU model we aware of. All experiments was performed on an Nvidia GTX 1060 GPU. The host contains 8 Intel cores running at 2.8Ghz, 16GB RAM and SSD hard drive.
