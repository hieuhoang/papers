...
We trained a sequence-to-sequence, encoder-decoder NMT system similar to that described in (Rico ???). This uses recurrent neural network with gated recurrent units. The input and output vocabulary size were both set to 85,000 using BPE ??? to adjust the vocabulary to the desired size. The hidden layer dimensions were kept at the default of 512. We used the Marian toolkit to train our models.

For inference, we used Amun (???), the current fastest open-source inference engine for the RNN/GRU model we aware of.

Our training data consisted of the German-English parallel sentences from the Europarl corpus (???). To test inference speed, we used two test set with differing characteristics:
\begin{enumerate}
...
\end{enumerate}
Table~\ref{tab:corpora} gives details of the training and test corpora.
