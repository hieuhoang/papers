...
In fact, we are only interested in the argmax class during inference, not the probability. Therefore, by using the argmax calculated in ??? rather than ???, we can avoid computing the softmax altogether. The outline of the our function, combining the work of ???-??? is shown in ???.
