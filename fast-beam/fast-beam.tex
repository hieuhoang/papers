...
As neural machine translation (NMT) models have become the new state-of-the-art, the challenge is to make their deployment efficient and economical. We have identified two areas where typical NMT models differ significantly from other deep-learning models that impact on their inference speed. Firstly, NMT models usualy contains a large number of output classes, corresponding to the output vocabulary. Secondly, rather than a single label, the output from machine translation models is a sequence of class labels that makes up the words or subwords in the target sentence. The sentence lengths are unpredictable, leading to inefficiencies during batch processing. We provide solutions for each of these issues which can increase batched inference speed by up to 41\% on modern GPUs without affecting model quality.
...
\end{tabular}
\caption{Actual batchsize during decoding (Europarl test set)}
...
There has been little work on batching algorithms, which is suprising considering its critical importance for fast model inference. ~\cite{Neubig-autobatching} describe an novel batching algorithm, however, its aim is to alleviate the burden on developers in batching of new models, not faster batching.
...
\end{tabular}
\caption{Translation speed for Europarl test set when using the fused kernel}
...
\end{tabular}
\caption{Translation speed for Opensubtitles test set when using the fused kernel}
...
\end{tabular}
\caption{Proportion of time spent during translation (Opensubtitles test set)}
...
\end{tabular}
\caption{Translation speed for Europarl test set using top-up batching}
...
\end{tabular}
\caption{Translation speed for OpenSubtitles test set using top-up batching}
...
\end{tabular}
\caption{Actual batchsize during decoding}
