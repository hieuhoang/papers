...
Figure~\ref{fig:mempool} compares the decoding speed for Moses and our decoder, using the same models, parameters and test set. Our decoder is over 3 times faster with one thread, and 5.0 times faster using all cores. However, like Moses, performance actually worsens after approximately 15 threads. %, however, the problem is not as pronounced. This gives us a better foundation on which to build further innovations for fast, multi-core decoding.
\begin{figure}[h]
\centering
\begin{tabular}{cc}
...
Our work is intended as a drop-in replacement for the Moses decoder. Given the same parameters and models, the model scores are comparable and the search space should be identical (or very nearly identical). This should lead to almost translation quality, which we verify with Figure~\ref{???}.
