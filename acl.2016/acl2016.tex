...
There are a number of phrase-based decoding implementations, many of which implements the phrase-based extensions above.
...
This paper focuses on overcoming the shortcomings of existing decoding implementations on multicore servers.
...
Of these feature functions, the last four are simple and do not leave much room for optimization. As have already been mentioned, the language model consumes a large part of decoding time but it has already been heavily optimized in previous works such as ~\newcite{Heafield-kenlm} and ~\newcite{yasuhara-EtAl:2013:EMNLP}.

Therefore, we will focus on optimizing the functionality of the phrase-table and lexicalized re-ordering model.
...
There are four main optimizations that we shall investigate. 
...
Thirdly, we will look at for optimizating the phrase-table, specifically in relation to the Probing implementation, focusing on two techniques. 

One, the default translation rule caching mechanism in Moses saves the most recently used rules. However, this is a complex mechanism which require locking when timestamping datastructures and periodic clearing of old rules in order in order to reduce memory consumption. It actually makes decoding slower, Table~\ref{tab:phrase-table-cache}. 
\begin{table}[h]
\small
\begin{center}
