...
We also note that distortion limit which constrains hypothesis extension is dependent on the hypothesis' coverage vector \emph{and} the end position of most recent source word that has been translated. The distortion limit must be checked for every instance of a hypothesis and translation rule, Figure~\ref{algo:Hypothesis Expansion with Unsorted Stack}.
\begin{figure} [h]
\begin{algorithmic}
%\REQUIRE source sentence $s$, translation options
\FORALL{ hypo in $stack$} 
  \FORALL{ translation rules}
      \IF {CanExpand(hypo coverage, hypo end position, translation rule range)}
...
\end{figure}
However, by separating hypotheses according to coverage and end position, the distortion limit check only needs to be done for each stack, potentially speeding up decoding, Figure~\ref{algo:Hypothesis Expansion with Sorted Stack}.
\begin{figure} [h]
\begin{algorithmic}
%\REQUIRE source sentence $s$, translation options
\FORALL{ coverage and end position in $stack$}
  \FORALL{ translation rules}
      \IF {CanExpand(coverage, end position, translation rule range)}
        \FORALL{hypo in [coverage and end position]}
  	  \STATE expand hypo with translation rule $\rightarrow$ new hypo
	  \STATE add new hypo into $next stack$
	\ENDFOR
      \ENDIF
