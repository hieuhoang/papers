...
However, we also note that distortion limit which constrains hypothesis extension is dependent on the hypothesis' coverage vector \emph{and} the end position of most recent source word that has been translated. Using a cardinality-based stack, the distortion limit must be checked for every instance, Figure~\ref{algo:Hypothesis Expansion with Unsorted Stack}.
\begin{figure} [h]
\begin{algorithmic}
%\REQUIRE source sentence $s$, translation options
\STATE insert empty hypothesis into $stack_0$
\FOR{$i = 0$ to $|s|-1$} 
  \STATE prune $stack_i$
  \FORALL{ hypothesis in $stack_i$} 
    \FORALL{ translation options}
       \IF {hypothesis and translation option are compatible}
         \STATE expand hypothesis with translation option $\rightarrow$ new hypothesis
         \STATE coverage of  new hypothesis $\rightarrow C$
         \STATE insert new hypothesis $\rightarrow stack_{|C|}$
         \STATE recombine new hypothesis if possible
         \STATE prune $stack_{|C|}$ if necessary
       \ENDIF
...
Therefore, if the hypotheses are separated by coverage vector and end position, the distortion limit constraint can be moved to an outer loop, potentially speeding up decoding.
