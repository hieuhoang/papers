...
Similar to the phrase-table, the lexicalized reordering model is trained on parallel data, a model file is created during training which is then queried during decoding. Inevitably, the querying results in some loss of decoding speed. Efforts such as~\newcite{junczys_tsd_2012b} have been made to increase the speed of the query.

However, the model's query key are the source and target phrase of each translation rule. Rather than storing the lexicalized reordering model separately, we shall integrating it into the translation model, eliminating the need to query a separate file.
