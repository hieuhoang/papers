...
Over 24\% of the Moses decoder running time is spent on memory management and this increases when more threads are used, Table~\ref{tab:optimizing-memory}, braking the scaling of the decoder to many cores. By contrast, our decoder spends 11\% on memory management and does not significantly increase with more cores.
\begin{table}[h]
\begin{center}
...
\begin{enumerate}
  ...
  \item coverage and end position of most recent translated source word.
\end{enumerate}
Table~\ref{tab:stack-configuration} present the decoding time (with 32 threads) for each stack configuration, and the average model on the held out test set.
\begin{table}[h]
\begin{center}
\begin{tabular}{|l|p{17mm}|p{17mm}|} \hline
			& Decoding \newline Time(secs)	& Avg model \newline score \\ \hline
Coverage cardinality  	& 186 	& -8.59922 \\
Coverage 		& ??? 	& ??? \\ 
Coverage \& end pos	& 196	& -8.59922 \\ \hline
\end{tabular}
\end{center}
\caption{Decoding time and average model scores for different stack configurations}
