...
For any realistic sized phrase-based model to be used in an online situation, memory and loading time constraints requires us to use load-on-demand phrase-table implementations. Moses contains a number of such implementations with different performance characterics, Figure~\ref{fig:moses-phrase-tables-time}.
\begin{figure}[h]
\centering
\begin{tabular}{cc}
{\includegraphics[scale=0.4]{moses-phrase-table.png}} 
\end{tabular}
\caption{Moses decoding time with different phrase-tables implementations}
\label{fig:moses-phrase-tables-time}
\end{figure} 
We will use the Probing phrase-table (???) exclusively in our work as this appears to have the fastest translation rule lookup, especially with large number of cores. We will look at two main areas for phrase-table optimization specifically in relation to the Probing implementation. 

Firstly, we will look at the \textbf{caching} of often used translation rules.

Secondly, the Probing phrase-table use a simple \textbf{compression} algorithm to compress the target side of the translation rule. We will look at the affect this has on decoding speed.
