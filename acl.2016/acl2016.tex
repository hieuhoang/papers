...
However, we also note that distortion limit which constrains hypothesis extension is dependent on the hypothesis' coverage vector \emph{and} the end position of most recent source word that has been translated. Therefore, if the hypotheses are separated by coverage vector \emph{and} end position, the distortion limit constraint can be moved to an outer loop, potentially speeding up decoding.
