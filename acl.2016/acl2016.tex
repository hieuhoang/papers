...
~\newcite{mfernandez2016boosting} was concerned with multi-core speed but treated the decoding process as a black box within a parallelization framework.
...
%The first four feature functions are simple and do not leave much room for optimization. As have already been mentioned, the language model consumes a large part of decoding time but it has already been heavily optimized in previous works such as ~\newcite{Heafield-kenlm} and ~\newcite{yasuhara-EtAl:2013:EMNLP}.
...
A memory pool is a large block of memory that has been pre-allocated by the operating system from which smaller blocks can be given out to the application. We will use thread-specific memory pools to avoid locking contention during memory access. Our memory pool will be dynamic, growing in size when required but never reducing in size.

To further increase memory management speed, objects in the memory pool are not deleted. Unused objects accumulates in the pool until the decoding of a sentence completes and the pool is simply reused for the decoding of the next sentence. Accumulating unused objects can result in unacceptably high memory usage so object queues are available for high-churn objects which allows the decoder to re-cycle some unused objects during the decoding of a particular sentence. LIFO queues are used so that the most recently accessed memory are used, increasing CPU cache hits.
...
Similar to the phrase-table, the lexicalized re-ordering model is trained on parallel data. A resultant model file is created in training which is then queried during decoding. Inevitably, the querying results in loss of decoding speed. Previous work such as~\newcite{junczys_tsd_2012b} improve querying speed with more efficient data structures.

However, the model's query keys are the source and target phrase of each translation rule. Rather than storing the lexicalized re-ordering model separately, we shall integrating it into the translation model, eliminating the need to query a separate file. This has precedent in ~\newcite{peitz2012jane} but the effect on decoding speed were not published. We will compare results with using a separate model in this paper.
